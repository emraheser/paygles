...
import { PlusCircle, Trash2, Globe, Clock3, MessageCircle, Tag, X, Power, Link2, Send, ShieldCheck } from "lucide-react";
...
export function Admin() {
    ...
    const handleManualLink = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setManualTitle("");
            setManualUrl("");
            alert("Link oluşturuldu, bildirim gönderilecek.");
        } catch {
            alert("Hata oluştu.");
        } finally {
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    <Button
                        ...
                        className="bg-cyan-500 hover:bg-cyan-400 text-white rounded-full h-9 px-4 text-sm font-medium"
                    >
                    ...
                    <Button
                        ...
                        className="bg-amber-500 hover:bg-amber-400 text-white rounded-full h-9 px-4 text-sm font-medium"
                    >
                    ...
                    <Button
                        ...
                        className="bg-emerald-500 hover:bg-emerald-400 text-white rounded-full h-9 px-4 text-sm font-medium"
                    >
                    ...
                    <Button
                        ...
                        className="bg-indigo-500 hover:bg-indigo-400 text-white rounded-full h-9 px-4 text-sm font-medium"
                    >
                ...
                <div className="flex items-center gap-1.5">
                    ...
                    <Input
                        ...
                        className="h-7 w-16 border-zinc-700 bg-zinc-800 px-2 text-xs text-center [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                    />
                    <span className="text-xs text-zinc-500">dk</span>
                    <Button
                        type="button"
                        onClick={handleSaveInterval}
                        className="h-7 bg-indigo-500 px-3 text-xs text-white hover:bg-indigo-400 rounded-lg"
                    >
                        Uygula
                    </Button>
                </div>
            ...
            {showDomainForm && (
                <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-4">
                    <p className="text-xs text-zinc-400">Sadece bu domainlerden gelen fırsat linkleri Telegram'a gönderilir. Boşsa tüm linkler gönderilir.</p>
                    <form onSubmit={handleAddDomain} className="flex items-center gap-2">
                        <Input
                            placeholder="Ör: hepsiburada.com, trendyol.com"
                            value={newDomain}
                            onChange={(e) => setNewDomain(e.target.value)}
                            className="h-10 border-zinc-700 bg-zinc-800 text-sm flex-1"
                        />
                        <Button
                            type="submit"
                    ...
                    <div className="flex flex-wrap gap-2">
                        ...
                        {domains.length === 0 && (
                            <p className="text-xs text-zinc-500">Henüz domain eklenmedi — tüm linkler gönderilecek.</p>
                        )}
                    </div>
                </div>
            )}
            ...
            {showKeywordForm && (
                <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-4">
                    <p className="text-xs text-zinc-400">Telegram kanallarından sadece bu keywordları içeren mesajlar alınır. Boşsa tüm mesajlar alınır.</p>
                    ...
                    <div className="flex flex-wrap gap-2">
                        ...
                        {keywords.length === 0 && (
                            <p className="text-xs text-zinc-500">Henüz keyword eklenmedi — tüm mesajlar alınacak.</p>
                        )}
                    </div>
                </div>
            )}
            ...
            {showManualForm && (
                ...
                >
                    ...
                    <div className="space-y-1.5">
                        <Label className="text-xs text-zinc-400 font-medium">Fırsat Linki</Label>
                        <Input
                            placeholder="https://..."
                            required
                            type="url"
            ...
            {showForm && (
                ...
                >
                    {/* Source Type Toggle */}
                    <div className="space-y-1.5">
                        <Label className="text-xs text-zinc-400 font-medium">Kaynak Türü</Label>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-1.5">
                            ...
                            />
                            {formData.source_type === "telegram" && (
                                <p className="text-[0.7rem] text-zinc-500">Üye olduğunuz Telegram kanalının kullanıcı adı veya linki</p>
                            )}
                        </div>
                    </div>
                    ...
                    {formData.source_type === "web" && (
                        <>
                            <div className="space-y-1.5">
                                ...
                                <Input placeholder=".structItem--thread" required value={formData.topic_list_selector} onChange={set("topic_list_selector")} className="bg-zinc-800 border-zinc-700 h-10 text-sm" />
                                <p className="text-[0.7rem] text-zinc-500">Her bir konuyu temsil eden tekrarlanan HTML elementi</p>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-1.5">
                                    <Label className="text-xs text-zinc-400 font-medium">Başlık Seçicisi</Label>
                                    <Input placeholder=".structItem-title a" required value={formData.title_selector} onChange={set("title_selector")} className="bg-zinc-800 border-zinc-700 h-10 text-sm" />
                                </div>
                                <div className="space-y-1.5">
                                    <Label className="text-xs text-zinc-400 font-medium">Link Seçicisi</Label>
                                    <Input placeholder=".structItem-title a" required value={formData.link_selector} onChange={set("link_selector")} className="bg-zinc-800 border-zinc-700 h-10 text-sm" />
                                </div>
                            </div>

                            <div className="space-y-1.5">
                                <Label className="text-xs text-zinc-400 font-medium">Konu Oluşturma Tarihi Seçicisi (Opsiyonel)</Label>
                                <Input
                                    ...
                                    className="bg-zinc-800 border-zinc-700 h-10 text-sm"
                                />
                                <p className="text-[0.7rem] text-zinc-500">Son mesaj değil, konu açılış tarihini veren alan</p>
                            </div>
                        </>
                    )}

                    <div className="flex justify-end gap-3 pt-2">
                        <Button type="button" variant="ghost" onClick={() => setShowForm(false)} className="text-zinc-400 text-sm">
                            İptal
                        </Button>
            ...
            <div className="space-y-3">
                {sites.map((site) => (
                    <div
                        key={site.id}
                        className="flex items-center justify-between bg-zinc-900 border border-zinc-800 rounded-xl px-5 py-4 hover:border-zinc-700 transition-colors cursor-pointer"
                    ...
                    >
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-zinc-500 truncate max-w-md">{site.url}</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
                            {site.source_type === "web" && site.topic_list_selector && (
                                <code className="text-[0.7rem] text-indigo-400 bg-indigo-400/10 px-2 py-1 rounded">
                ...
                {sites.length === 0 && (
                    <div className="flex flex-col items-center justify-center py-20 bg-zinc-900/50 rounded-2xl border border-zinc-800">
                        ...
                        <p className="text-zinc-500 text-sm">Henüz kaynak eklenmedi</p>
                    </div>
                )}
            </div>
                    ...
                    <form onSubmit={handleUpdate} className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                                ...
                                <Input required value={editFormData.name} onChange={setEdit("name")} className="bg-zinc-800 border-zinc-700 h-10 text-sm" />
                            </div>
                            <div className="space-y-1.5">
                                <Label className="text-xs text-zinc-400 font-medium">
                                    {editFormData.source_type === "telegram" ? "Kanal Kullanıcı Adı veya Linki" : "Hedef URL"}
                                </Label>
                                <Input required value={editFormData.url} onChange={setEdit("url")} className="bg-zinc-800 border-zinc-700 h-10 text-sm" />
                            </div>
                        </div>
                        ...
                        {editFormData.source_type === "web" && (
                            <>
                                <div className="space-y-1.5">
                                    <Label className="text-xs text-zinc-400 font-medium">Liste CSS Seçicisi</Label>
                                    <Input required value={editFormData.topic_list_selector} onChange={setEdit("topic_list_selector")} className="bg-zinc-800 border-zinc-700 h-10 text-sm" />
                                </div>

                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="space-y-1.5">
                                        <Label className="text-xs text-zinc-400 font-medium">Başlık Seçicisi</Label>
                                        <Input required value={editFormData.title_selector} onChange={setEdit("title_selector")} className="bg-zinc-800 border-zinc-700 h-10 text-sm" />
                                    </div>
                                    <div className="space-y-1.5">
                                        <Label className="text-xs text-zinc-400 font-medium">Link Seçicisi</Label>
                                        <Input required value={editFormData.link_selector} onChange={setEdit("link_selector")} className="bg-zinc-800 border-zinc-700 h-10 text-sm" />
                                    </div>
                                </div>
